        ...
        for (int y=0;y<len;y++){
            ...
            for (int d1=1;d1<len;d1++){
                for (int d2=1;d2<len;d2++){
                    if(x+d1+d2>len-1 or y+d1+d2>len-1) continue;
                    if ( x<x+d1+d2 && x+d1+d2<=(len-1) && 0<=y-d1 && y-d1<y && y+d2<=len-1){
...
int main(void){
    ...
    // data_d=new int*[len];
    // for (int i=0;i<len;i++){
    //     data_d[i]=new int[len];
    //     memset(data_d[i],0,sizeof(int)*len);
    // }
    // zonemap=new int*[len];
    // for (int i=0;i<len;i++){
