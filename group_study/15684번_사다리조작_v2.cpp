...
int find_combi(){
    ...
    for (int i=0;i<h;i++){
        for (int j=0;j<n;j++){
            if (data_in[i][j]==false and j<n-1){
                combis tmp;
                tmp.num=1;
                tmp.datas[0]={i,j};
                v.push_back(tmp);
    ...
    for (int i=0;i<first_cnt-2;i++){
        combis tmp;
            tmp.num=3;
            tmp.datas[0]=v.at(i).datas[0];
            // {v.at(i).datas[0].first,v.at(i).datas[0].second};
        for (int j=i+1;j<first_cnt-1;j++){
            // tmp.datas[1]={v.at(j).datas[0].first,v.at(j).datas[0].second};
            tmp.datas[1]=v.at(j).datas[0];
            for (int k=j+1;k<first_cnt;k++){
                tmp.datas[2]=v.at(k).datas[0];
